...
global $wpdb, $table_name_group, $table_name_moderation, $table_name_message, $table_name_requestmanager, $table_name_requestmanager_taxonomy, $table_name_user_taxonomy, $table_name_parsed_emails, $table_name_sent_emails, $table_name_users, $table_name_usermeta;
...
$table_name_moderation              = $wpdb->prefix . "mailing_group_moderation";
...
function wpmg_add_mailing_group_plugin()
{
    ...
    $MSQL = "show tables like '$table_name_moderation'";
    if ($wpdb->get_var($MSQL) != $table_name_moderation) {
        $sql = "CREATE TABLE IF NOT EXISTS `$table_name_moderation` (

			  `mod_id` int(9) NOT NULL AUTO_INCREMENT,

			  `id` int(9) NOT NULL,			  

			  `moderation` varchar(10) NOT NULL DEFAULT 'off',

			  `mod_type` varchar(10) NOT NULL,

			  `mod_duration` int(3) NOT NULL,

			  `mod_member` text NOT NULL,
			  
			  `mod_text` text NOT NULL,
			  
			  `mod_email` varchar(255) NOT NULL,
...
function wpmg_mailinggroup_Menu()
{
    ...
    if ($status =='valid') {
    	...
    	add_submenu_page('wpmg_mailinggroup_intro',__('Message Moderation', 'mailing-group-module'), __('Message Moderation', 'mailing-group-module'), $admin_level, 'wpmg_mailinggroup_message_moderation', 'wpmg_mailinggroup_message_moderation' );
        ...
        add_menu_page(__('Mailing Group Manager', 'mailing-group-module'), __('Mailing Group Manager', 'mailing-group-module'), $admin_level, 'wpmg_mailinggroup_license', 'wpmg_activate_license_page');
...
function wpmg_mailinggroup_message_moderation()
{
    global $wpdb, $objMem, $table_name_group, $table_name_sent_emails, $table_name_parsed_emails, $table_name_user_taxonomy;
    include "template/mg_messagemoderation.php";
}
...
add_action('wp_ajax_wpmg_test_email_conn', 'wpmg_test_email_conn_callback');
/* Short codes for ajax requests */
/* callback function for above ajax requests */
function wpmg_test_email_conn_callback()
{
    global $wpdb, $objMem, $table_name_group;
	include "template/mg_testemailconnection.php";
...
{
    global $wpdb, $objMem, $table_name_group, $table_name_moderation, $table_name_user_taxonomy, $table_name_sent_emails;
    include "template/mg_mailinggroupadd.php";
    die(); /*  this is required to return a proper result */
}
function wpmg_mailinggrouplisting_callback()
{
    global $wpdb, $objMem, $table_name_group, $table_name_moderation;
    include "template/mg_mailinggrouplist.php";
    die(); /*   this is required to return a proper result */
}
function wpmg_addmailgroupsetting_callback()
{
    global $wpdb, $objMem, $_POST, $table_name_group, $table_name_moderation; 
    ...
    if ($addme == 1) {
        if (!$objMem->checkRowExists($table_name_group, "title", $_POST, "")) {
		    $gid = $objMem->addNewRow($table_name_group, $_POST, $myFields);
			$_POST['id'] = $gid;
		    $objMem->addNewRow($table_name_moderation, $_POST, $modFields);
		    echo "added";
            exit;
        } else {
            echo "exists";
        ...
        if (!$objMem->checkRowExists($table_name_group, "title", $_POST, "idCheck")) {
		    if($_POST['mod_gid'] !=''){
			    $objMem->updRow($table_name_moderation, $_POST, $modFields);
			}else{
			    $objMem->addNewRow($table_name_moderation, $_POST, $modFields);
			}
...
function wpmg_human_time_diff( $from, $to = '' ) {
    if ( empty( $to ) )
        $to = time();
