...
jQuery(document).ready(function(){
	...
	jQuery("#moderationrequest").submit(function(){
		if (!isCheckedById("selector")) {
			alert("Please select atleast one message to continue.");
			return false;
		} else {
			if(jQuery("#mass_action").val()=='' && jQuery("#mass_action2").val()=='') {
				alert("Please select atleast one action from the dropdown.");
				return false;	
			} else if(jQuery("#mass_action").val()=='1' || jQuery("#mass_action2").val()=='1') {
				return confirm('Are you sure you want to approve these message(s)?');
			} else if(jQuery("#mass_action").val()=='2' || jQuery("#mass_action2").val()=='2') {
				return confirm('Are you sure you want to delete these message(s)?');
			} else if(jQuery("#mass_action").val()=='3' || jQuery("#mass_action2").val()=='3') {
				return confirm('Are you sure you want to delete these message(s) and inform the sender?');
			}
		}
		return true;
	});		
